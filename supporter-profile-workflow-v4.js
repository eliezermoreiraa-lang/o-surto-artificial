(() => {
  ...
  function client(){
    ...
    sb=window.getSurtoSupabaseClient(SB_URL,SB_KEY,{auth:{persistSession:true,autoRefreshToken:true,detectSessionInUrl:false,flowType:'implicit',storageKey:'surto-auth'}});
    return sb;
  }
  ...
  function css(){
    ...
    const s=document.createElement('style');s.id='sv4-css';s.textContent=`
      ...
      .sv4-form{display:grid;gap:15px;max-width:760px;margin-top:18px}.sv4-required-note{margin:0!important;font-size:10px!important;color:rgba(245,245,245,.55)!important}.sv4-required{color:#ff3945;font-weight:700}.sv4-optional{color:rgba(245,245,245,.38);font-size:9px;margin-left:5px;text-transform:uppercase;letter-spacing:.08em}.sv4-hint{margin:-1px 0 0;font-size:10.5px;line-height:1.45;color:rgba(245,245,245,.43)}.sv4-field{display:grid;gap:7px}.sv4-field label,.sv4-check{font-size:11px;color:rgba(245,245,245,.58)}.sv4-field input,.sv4-field select{width:100%;height:46px;border:0;border-radius:8px;padding:0 13px;background:#0B0D13;color:#F5F5F5;box-shadow:inset 0 0 0 1px rgba(245,245,245,.16);font:13px Inter,sans-serif}.sv4-check{display:flex;gap:9px;align-items:flex-start;line-height:1.5}
  ...
  function profileOk(p){return !!(p?.display_name&&p?.social_network&&p?.social_handle&&p?.notification_email&&p?.public_consent)}
  ...
  async function renderEditable(root,m){
    ...
    root.innerHTML=`<div class="sv4"><h1 class="sv4-title">Perfil de Divulgação</h1><p class="sv4-lead">Preencha seus dados, salve a primeira etapa e depois envie as referências que a produção usará para criar seu avatar.</p>
      <section class="sv4-step"><div class="sv4-kicker">ETAPA 1 DE 2</div><h2>PERFIL DE DIVULGAÇÃO</h2><p>Essas informações serão usadas quando você aparecer nas produções do Surto.</p><div class="sv4-form">
        <p class="sv4-required-note"><span class="sv4-required">*</span> Campos obrigatórios</p>
        <div class="sv4-field"><label for="sv4-name">Nome de divulgação <span class="sv4-required" aria-hidden="true">*</span></label><input id="sv4-name" required aria-required="true" value="${esc(p.display_name||m.user?.displayName||'')}"></div>
        <div class="sv4-field"><label for="sv4-network">Rede social <span class="sv4-required" aria-hidden="true">*</span></label><select id="sv4-network" required aria-required="true"><option value="">Selecione</option>${opts.map(o=>`<option value="${o[0]}" ${net===o[0]?'selected':''}>${o[1]}</option>`).join('')}</select></div>
        <div class="sv4-field"><label for="sv4-handle">@ / nome de usuário <span class="sv4-required" aria-hidden="true">*</span></label><input id="sv4-handle" required aria-required="true" value="${esc(p.social_handle?'@'+String(p.social_handle).replace(/^@/,''):'')}" placeholder="@seuperfil"></div>
        <div class="sv4-field"><label for="sv4-email">E-mail para notificações <span class="sv4-required" aria-hidden="true">*</span></label><input id="sv4-email" type="email" required aria-required="true" value="${esc(p.notification_email||m.user?.email||'')}"></div>
        <div class="sv4-field"><label for="sv4-url">Link do perfil <span class="sv4-optional">opcional</span></label><input id="sv4-url" type="url" value="${esc(p.social_url||'')}" placeholder="https://..."><p class="sv4-hint">Opcional. Informar o link facilita a divulgação e evita erros ao localizar o seu perfil.</p></div>
        <label class="sv4-check"><input id="sv4-consent" type="checkbox" required aria-required="true" ${p.public_consent?'checked':''}> <span>Autorizo a exibição destes dados nas divulgações e no mural de apoiadores. <span class="sv4-required" aria-hidden="true">*</span></span></label>
  ...
  function bindEditable(root){
    const save=root.querySelector('#sv4-save');if(save)save.addEventListener('click',async()=>{const msg=root.querySelector('#sv4-profile-msg');const body={displayName:root.querySelector('#sv4-name').value.trim(),socialNetwork:root.querySelector('#sv4-network').value,socialHandle:root.querySelector('#sv4-handle').value.trim(),notificationEmail:root.querySelector('#sv4-email').value.trim(),socialUrl:root.querySelector('#sv4-url').value.trim(),publicConsent:root.querySelector('#sv4-consent').checked};if(!body.displayName||!body.socialNetwork||!body.socialHandle||!body.notificationEmail||!body.publicConsent){msg.className='sv4-msg';msg.textContent='Preencha todos os campos marcados com *.';return}save.disabled=true;save.textContent='SALVANDO…';const c=client();const {data,error}=await c.functions.invoke('supporter-profile-save',{body});if(error||!data?.ok){msg.className='sv4-msg';msg.textContent=data?.error||'Não foi possível salvar agora.';save.disabled=false;save.textContent='SALVAR ETAPA';return}msg.className='sv4-msg ok';msg.textContent='Perfil salvo com sucesso ✓';model=await load(true);setTimeout(()=>renderProfile(root,model),350)});
  ...
  async function renderProfile(root,m){if(!root||!m?.currentSupport||!['supporter','highlight','vip'].includes(m.currentSupport.tier)||root.dataset.sv4Busy==='1')return;model=m;root.dataset.sv4Busy='1';try{if(m.publicityProfile?.submission_completed_at)await renderCompleted(root,m);else await renderEditable(root,m)}finally{root.dataset.sv4Busy='0'}}
  ...
  async function enhance(){
    if(busy)return;busy=true;try{css();const root=document.querySelector('#surto-supporter-real-v2');if(!root)return;const m=await load(false);if(!m?.currentSupport||!['supporter','highlight','vip'].includes(m.currentSupport.tier))return;const h=norm(root.querySelector('h1')?.textContent);
