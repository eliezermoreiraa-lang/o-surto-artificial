(() => {
  ...
  function ensureCss(){
    ...
    style.textContent = `
      ...
      @media(max-width:900px){.sd-checkout-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(max-width:620px){.sd-vip-points{grid-template-columns:1fr}}@media(max-width:520px){.sd-checkout-grid{grid-template-columns:1fr}.sd-plan-card{min-height:0}.sd-locked{padding:24px}}
    `;
  ...
  function bindRoot(root){
    ...
    if(saveVipBriefing)saveVipBriefing.addEventListener('click',async()=>{
      ...
      const body={promotionGoal:root.querySelector('#sd-vip-goal')?.value.trim()||'',sceneIdea:root.querySelector('#sd-vip-scene')?.value.trim()||'',productOrMaterial:root.querySelector('#sd-vip-product')?.value.trim()||'',additionalNotes:root.querySelector('#sd-vip-notes')?.value.trim()||'',termsAccepted:!!root.querySelector('#sd-vip-terms')?.checked};
      ...
      try{const sb=ensureClient();const {data,error}=await sb.functions.invoke('vip-briefing-save',{body});if(error||!data?.ok)throw new Error(data?.error||'briefing_save_failed');await loadData(true);msg.className='sd-msg ok';msg.textContent='Briefing enviado para a produção ✓';schedule(false)}catch(e){msg.className='sd-msg';msg.textContent='Não foi possível salvar agora. Tente novamente.';saveVipBriefing.disabled=false;saveVipBriefing.textContent='TENTAR NOVAMENTE'}
    });
