...
(() => {
  ...
  function bind(root,{client,open,refresh,tier}){
    ...
    root.querySelector('#sd-monthly-form')?.addEventListener('submit',async event=>{
      ...
      try{const out=await invoke(client(),{action:'create',tier,method:'cartao',consent:true,consentVersion:'monthly-card-2026-09-25-promo',expectedAmount:Number(event.target.dataset.expectedAmount)});const url=new URL(out.invoiceUrl);if(url.protocol!=='https:'||url.hostname!=='asaas.com')throw new Error('Endereço de pagamento inválido.');window.location.assign(url.href)}
