...
Deno.serve(async (req: Request) => {
  ...
  if (results.some(result => result.error)) return new Response(JSON.stringify({ error: "Não foi possível carregar seus dados. Tente novamente." }), { status: 503, headers: cors(req) });
  const [{ data: profile }, { data: supports }, { data: subscriptions }, { data: publicity }, { data: vipBriefings }] = results;
