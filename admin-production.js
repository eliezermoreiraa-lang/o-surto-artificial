(() => {
  ...
  async function openSupporter(userId, supportId) {
    ...
    try {
      const m=maps(); const supports=data.supports.filter(x=>x.user_id===userId); const support=supports.find(x=>x.id===supportId)||supports[0]||{}; const p=m.publicity[userId]||{}; const a=data.appearances.find(x=>x.support_id===support.id)||{}; const profile=m.profiles[userId]||{}; const user=m.users[userId]||{}; const signed={face:null,body:null};
