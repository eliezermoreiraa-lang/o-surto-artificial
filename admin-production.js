(() => {
  ...
  function monthlyOverview(){
    const rows=data.subscriptions||[],m=maps();
    const active=rows.filter(x=>x.status==='active');
    const states={pending:'Aguardando contratação',active:'Ativa',past_due:'Pagamento pendente / pausada',cancelled:'Cancelada',expired:'Expirada'};
    return `<section class="panel panel-pad" style="margin-top:24px"><div class="section-head"><div><h2>Assinaturas mensais</h2><p>${active.length} ativa(s) · ${money(active.reduce((sum,s)=>sum+Number(s.amount),0))}/mês contratados. Previsão, não valor recebido.</p></div></div><div class="table-wrap"><table class="table"><thead><tr><th>APOIADOR</th><th>PLANO</th><th>MENSALIDADE</th><th>SITUAÇÃO</th><th>PRÓXIMA COBRANÇA</th></tr></thead><tbody>${rows.length?rows.map(s=>`<tr><td>${e(person(s,m).name)}</td><td>${e(labels[s.tier])}</td><td>${money(s.amount)}</td><td>${e(states[s.status]||s.status)}</td><td>${['cancelled','expired'].includes(s.status)?'—':date(s.next_due_date)}</td></tr>`).join(''):'<tr><td colspan="5" class="empty">Nenhuma assinatura contratada.</td></tr>'}</tbody></table></div><p class="finance-note">Cada mensalidade confirmada aparece também em Pagamentos e no relatório do mês. Cobranças futuras não contam como recebidas.</p></section>`;
  }
