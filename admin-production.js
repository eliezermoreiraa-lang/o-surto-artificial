(() => {
  ...
  function vipBriefingSection(support, briefing) {
    ...
    return `<section class="detail-section full"><div class="briefing-head"><div><h3>BRIEFING VIP DA CENA</h3><small>Enviado em ${dateTime(briefing.submitted_at||briefing.updated_at)}</small></div>${badge(briefing.status||'submitted')}</div><div class="briefing-grid">${item('O que deseja divulgar',briefing.promotion_goal)}${item('Como imagina a cena',briefing.scene_idea)}${item('Produto ou material em cena',briefing.product_or_material)}${item('Outras observações',briefing.additional_notes)}</div><p class="briefing-confirm">✓ O apoiador confirmou que entendeu o formato promocional ao final do episódio.</p></section>`;
  }
  function stats() {
