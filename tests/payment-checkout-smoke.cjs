...
const catalogue = fs.readFileSync(path.join(root, 'club-support.js'), 'utf8');
...
assert.equal(catalogue.includes('Apoio sem divulgação e sem aparição.'), true);
assert.equal(catalogue.includes('não é participação como personagem'), true);
